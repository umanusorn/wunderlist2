...
public class AsyncUpdateCart extends AsyncTask<String, String, String> {
...
public
AsyncUpdateCart ( Activity activity ) {
...
void onPostExecute ( String result ) {
	...
	delegate.processFinish ( result, "updatecart" );
...
String doInBackground ( String... params ) {
	uRL = activity.getApplicationContext ().getResources ().getString ( R.string.server_url ) +
	      activity.getApplication ().getResources ().getString ( R.string.update_cart );
	String outPut = "";
	//JSONObject jObject=new JSONObject();
		...
		try {
			...
			JSONArray items=new JSONArray (params[0]);
