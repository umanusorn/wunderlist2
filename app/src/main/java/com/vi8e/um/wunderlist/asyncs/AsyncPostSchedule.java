...
public class AsyncPostSchedule extends AsyncTask<String, String, String> {
...
public
AsyncPostSchedule ( Activity activity ) {
...
void onPostExecute ( String result ) {
	...
	delegate.processFinish ( result, "postschedule" );
...
String doInBackground ( String... params ) {
	uRL = activity.getApplicationContext ().getResources ().getString ( R.string.server_url ) +
	      activity.getApplication ().getResources ().getString ( R.string.post_schedule );
	String outPut = "";
	//JSONObject jObject=new JSONObject();
		...
		try {
			...
			JSONObject shippingmethod=new JSONObject (params[1]);
			jParams.put("ShippingMethodForm", shippingmethod);
			JSONObject param=new JSONObject (params[0]);
			jParams.put("ShippingInformationForm", param);
		...
		try{
			...
			Log.i ( "output post schedule", outPut );
