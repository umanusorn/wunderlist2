...
public class AsyncAddShoppingListItem extends AsyncTask<String, String, String> {
...
public
AsyncAddShoppingListItem ( Activity activity ) {
...
void onPostExecute ( String result ) {
	...
	delegate.processFinish ( result, "additem" );
...
String doInBackground ( String... params ) {
	uRL = activity.getApplicationContext ().getResources ().getString ( R.string.server_url ) +
	      activity.getApplication ().getResources ().getString ( R.string.add_shoppinglist_items );
	String outPut = "";
		...
		try {
			...
			jParams.put("list_id", params[0]);
			JSONArray items=new JSONArray (params[1]);
			jParams.put("items", items);
