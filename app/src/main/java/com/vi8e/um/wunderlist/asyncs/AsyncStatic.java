...
public class AsyncStatic extends AsyncTask<String, String, String> {
...
void onPostExecute ( String result ) {
	...
	delegate.processFinish ( result, "static" );
...
String doInBackground ( String... params ) {
	uRL = activity.getApplicationContext ().getResources ().getString ( R.string.server_url ) +
	      activity.getApplication ().getResources ().getString ( R.string.get_page );
	String outPut = "";
	//JSONObject jObject=new JSONObject();
		JSONObject jParams=new JSONObject ();
		try {
			//jObject.put("auth", Utils.auth);
			jParams.put("page_slug", params[0]);
