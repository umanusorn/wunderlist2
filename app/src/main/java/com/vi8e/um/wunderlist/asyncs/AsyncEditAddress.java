...
public class AsyncEditAddress extends AsyncTask<String, String, String> {
...
public
AsyncEditAddress ( Activity activity ) {
...
void onPostExecute ( String result ) {
	...
	delegate.processFinish ( result, "updateaddress" );
...
String doInBackground ( String... params ) {
	uRL = activity.getApplicationContext ().getResources ().getString ( R.string.server_url ) +
	      activity.getApplication ().getResources ().getString ( R.string.update_address );
	String outPut = "";
	JSONObject jParams =new JSONObject ();
		try {
			//jObject.put("auth", Utils.auth);
			jParams.put("customer_id", session.getCustomerId());
			jParams.put("customer_address_id", params[1]);
			JSONObject address=new JSONObject (params[0]);
			jParams.put("Address", address);
