...
public
AsyncSearch ( Activity activity, String type ) {
...
void onPostExecute ( String result ) {
	...
	delegate.processFinish ( result, "search" );
...
String doInBackground ( String... params ) {
	uRL = activity.getApplicationContext ().getResources ().getString ( R.string.server_url ) +
	      activity.getApplication ().getResources ().getString ( R.string.get_product_search );
	String outPut = "";
	//JSONObject jObject=new JSONObject();
		...
		try {
			...
			jCriteria.put("keywords", params[0]);
			if(params.length>1){
				if(type.equals("category")){
					jCriteria.put("category_id", params[1]);
				}else if(type.equals("brand")){
					jCriteria.put("brand_id", params[1]);
				}
			}
			jParams.put("criteria", jCriteria);
