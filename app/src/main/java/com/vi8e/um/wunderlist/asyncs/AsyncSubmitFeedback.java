...
public
AsyncSubmitFeedback ( Activity activity ) {
...
void onPostExecute ( String result ) {
	...
	delegate.processFinish ( result, "submitfeedback" );
...
String doInBackground ( String... params ) {
	uRL = activity.getApplicationContext ().getResources ().getString ( R.string.server_url ) +
	      activity.getApplication ().getResources ().getString ( R.string.post_feedback_form );
	String outPut = "";
	//JSONObject jObject=new JSONObject();
		...
		try {
			...
			jParams.put("name", params[0]);
			jParams.put("email", params[1]);
			jParams.put("phone", params[2]);
			jParams.put("topic", params[3]);
			jParams.put("subject", params[4]);
			jParams.put("message", params[5]);
